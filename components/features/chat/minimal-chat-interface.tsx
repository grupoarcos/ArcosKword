...
export function MinimalChatInterface() {
  ...
  const downloadAsPDF = async (content: string, messageId: string) => {
    try {
      console.log(
        "[v0] Starting PDF generation for content length:",
        content.length
      );
      console.log(
        "[v0] Full content preview:",
      ...
      const htmlContent = content
        ...
        .replace(/(\|[^\n]*\|(?:\r?\n\|[^\n]*\|)*)/g, (match) => {
          ...
          if (separatorIndex === -1) {
            console.log(
              "[v0] No separator found, treating first line as header"
            );
          ...
          const headerLines =
            separatorIndex === 0 ? [lines[0]] : lines.slice(0, separatorIndex);
          const dataLines =
            separatorIndex === 0
              ? lines.slice(1)
  ...
  const parseInlineSuggestions = (content: string) => {
    ...
    if (suggestions.length === 0) {
      ...
      for (
        ...
        i >= 0 && potentialSuggestions.length < 5;
        i--
      ) {
        const line = lines[i];
        ...
        if (line.includes(".") && line.endsWith(".") && line.length > 100)
          continue;
      ...
      if (potentialSuggestions.length > 0) {
        ...
        const filteredLines = contentLines.filter(
          (line) => !suggestionLines.has(line.trim())
        );
          ...
          {messages.length === 0 && (
            <div className="text-center py-4 sm:py-12">
              <h2 className="font-serif text-base sm:text-xl font-bold text-foreground mb-1 sm:mb-2">
                Welcome to Articollo
              </h2>
              <p className="text-muted-foreground text-xs sm:text-base px-4 mb-4 sm:mb-8">
                I'm here to help you with research, content creation, and
                amplification strategies.
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 sm:gap-3 max-w-5xl mx-auto px-2">
                {[
                  ...
                  <Button
                    ...
                    className="h-auto py-1.5 px-2 sm:p-3 text-left hover:bg-accent hover:text-accent-foreground transition-colors border border-gray-300 dark:border-gray-600 hover:border-primary/50 rounded-lg text-[10px] sm:text-sm leading-tight"
                    disabled={isLoading}
                  >
                  ...
                  >
                    {message.type === "audio" && message.audioUrl ? (
                      ...
                      <>
                        ...
                        <div className="text-sm sm:text-base leading-relaxed prose prose-sm max-w-none prose-headings:text-foreground prose-p:text-foreground prose-strong:text-foreground prose-em:text-foreground prose-ul:text-foreground prose-ol:text-foreground prose-li:text-foreground prose-code:text-foreground prose-code:bg-muted prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-code:text-xs prose-table:text-foreground">
                          <ReactMarkdown
                            ...
                            components={{
                              ...
                              p: ({ children }) => (
                                <p className="mb-2 sm:mb-3 last:mb-0 leading-relaxed text-foreground">
                                  {children}
                                </p>
                              ),
                              hr: () => (
                        ...
                        message.type !== "audio" && (
                          <div className="flex items-center gap-1">
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                copyToClipboard(cleanContent, message.id)
                              }
                            ...
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                downloadAsPDF(cleanContent, message.id)
                              }
                              className="h-6 px-2 text-xs hover:bg-muted hover:text-foreground transition-colors opacity-70 hover:opacity-100"
          ...
          <form onSubmit={handleSubmit}>
            <div className="flex gap-2 sm:gap-3">
              <Input
                ...
                placeholder="Ask Keyo to help with research, content creation, or amplification..."
              ...
              <Button
                ...
                className={`h-11 sm:h-12 w-11 sm:w-12 rounded-xl ${
