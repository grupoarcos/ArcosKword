  ...
  try {
    ...
    if (body.type === "voice" && body.audioData) {
      try {
        ...
        console.log(
          "[v0] Sending voice message to webhook with country:",
          body.country,
          "language:",
          body.language
        );
        const response = await fetch(
          "https://auto.arcosscale.com.br/webhook/articollo",
          {
            method: "POST",
            body: formData,
        ...
        return NextResponse.json(
          {
            success: false,
            error: `Audio processing failed: ${audioError.message}`,
          },
          { status: 500 }
        );
      }
      ...
      console.log("[v0] Sending text message to webhook:", {
        ...
        message:
          webhookPayload.message.substring(0, 50) +
          (webhookPayload.message.length > 50 ? "..." : ""),
      ...
      if (data.output && typeof data.output === "object") {
        responseMessage =
          data.output.output || data.output.message || "Response received";
        suggestions = data.output.suggestions || [];
      } else {
        responseMessage =
          data.output || data.message || data.response || "Response received";
      }
